...
to setup-parameters
  set strategy-list [ "C" "D" "T" "V" "R" "G" "S" "K" ]
end
...
; percent liklihood that strategy remains = percent of strategy occupying the population count

to go
  if ( ticks < stop-at or stop-at = 0 ) [
    ask turtles [ lt random 60 rt random 60 fd 1 ]
    ask turtles [
      if ( any? other turtles-here ) [
        interact-with one-of other turtles-here ]]

    ;ask min-n-of 30 turtles [ points ] [ set strategy replace-item ( random length strategy ) strategy ( one-of strategy-list ) ]
...
to interact-with [ target ]
  let ego self
  let ego-strategy [ get-strategy target ] of self
  let target-strategy [ get-strategy ego ] of target
...
to-report get-strategy [ target ]
  let current-letter item ( remainder ticks length strategy ) strategy
  let my-strategy ""
  carefully [ set my-strategy run-result ( word current-letter " " target ) ] [ set my-strategy error-message ]
  report my-strategy
end

to-report C [target] ; ALWAYS COOPERATE
  report "C"
end

to-report D [target] ; ALWAYS DEFECT
  report "D"
end

to-report T [target] ; TIT FOR TAT
  report first table:get-or-default memory [name] of target "C"
end

to-report V [target] ; REVERSE TIT FOR TAT
  report ifelse-value (( first table:get-or-default memory [name] of target "C" ) = "D" ) [ "C" ] [ "D" ]
end

to-report R [target] ; RANDOM
  report one-of [ "C" "D" ]
end

to-report G [target] ; GRIM
  report ifelse-value ( member? "D" table:get-or-default memory [name] of target "D" ) [ "D" ] [ "C" ]
end

to-report S [target] ; SOFT MAJORITY
...
to-report K [target] ; HARD MAJORITY
  let string table:get-or-default memory [name] of target ""
  let C-count 0
  let D-count 0
  foreach string [ char ->
...
;to-report O [target] ; TIT FOR TWO TATS
;  ;report one-of [ "C" "D" ]
;end
;
;to-report P [target]; TAT FOR TWO TITS
; ; report ifelse-value ( member? "D" table:get-or-default memory [name] of target "D" ) [ "D" ] [ "C" ]
;end
;
;
;to-report H [target]; HANDSHAKE
;  ;report ifelse-value ( member? "DCD" table:get-or-default memory [name] of target "C" ) [ "D" ] [ "C" ]
;end
...
to output-standings
  if ( remainder ticks 10 = 0 ) [
    ...
    foreach sorted-turtles [ guy ->
      ifelse ( rank < 31 )
      [ ask guy [ output-print (word rank "  " name "  " strategy "  " points ) ]]
